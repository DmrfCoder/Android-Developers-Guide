...
Activity和Fragment生命周期的最显着的差异是如何将其存储在其各自的后台堆栈中。默认情况下，Activity停止（stopped）时被置于由系统管理的Activity的后堆栈中（以便用户可以使用“Back”按钮导航回到它，如 [理解Task和回退栈](../理解Task和回退栈.md) 中所述）。但是，只有当您在执行删除Fragment的事务时通过调用addToBackStack（）显式请求保存实例时，系统才会将Fragment放入由宿主Activity管理的后台堆栈中。
